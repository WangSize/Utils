package com.norca.casing.dynamicresource;
...
public class ResourceManager {
    ...
    public static class UnInstalled {
        ...
        public Drawable getDrawable(String packageName, String fieldName) {
            ...
            int resourceID = getResourceID(packageName, "mipmap", fieldName);
            LoadedResource recource = getUnInstalledRecource(packageName);
            if (recource != null) {
        ...
        public String getString(String packageName, String fieldName){
            String string = null;
            int stringID = getResourceID(packageName, "string", fieldName);
            LoadedResource recource = getUnInstalledRecource(packageName);
            if (recource != null) {
                string = recource.resources.getString(stringID);
            }
            return string;
        }
                ...
                if (loadResource == null) {
                    try {
                        ...
                        loadResource.classLoader = new DexClassLoader(resourcePath, mDexDir, null,
                                ClassLoader.getSystemClassLoader());    //	设置正确的类加载器, 因为需要去加载R文件
        ...
        private PackageInfo queryPackageInfo(String resourcePath) {
            return mContext.getPackageManager().getPackageArchiveInfo(resourcePath, 0);
        }
